...
if that doesn't work try using `pip3` instead of `pip`. And if that doesn't work do the following:

```bash
python3 -m pip install -r VC/requirements.txt
```
...
### Generating Zeta Indexes

The zeta indexes are generated by running the `feature_completion.py` script. The script will generate the zetas for the given test suite and attach the results a the `test-completion-matrix-n.csv` file. Where n is the feature number.

To generate the zetas run the following command. 

```bash
cd VC/tests && python feature_compeletion.py -f 1
```
### Automated Testing
